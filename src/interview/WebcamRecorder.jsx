...
export default function WebcamRecorder({
    ...
    useEffect(() => {
        ...
        const onVisibilityChange = () => {
            if (!document.hidden || tabReportedRef.current) return;
            tabReportedRef.current = true;
    ...
    useEffect(() => {
        if (!faceMonitorEnabled || !recording) return;
        timerRef.current = setTimeout(sendFrame, FACE_START_DELAY);
    ...
    async function sendFrame() {
        ...
        const now = Date.now();
        if (now - lastSendRef.current < FACE_INTERVAL_MS) return;
        lastSendRef.current = now;
        ...
        timerRef.current = setTimeout(sendFrame, FACE_INTERVAL_MS);
    }
    ...
    return (
        <div className="webcam-glass-shell">
            ...
            {!recording ? (
                <button onClick={() => setRecording(true)}>Start Interview</button>
            ) : (
                <button onClick={() => window.dispatchEvent(new Event("stopInterview"))}>
                    Stop Interview
                </button>
            )}
        </div>
